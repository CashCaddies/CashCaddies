-- Leaderboard email from public.profiles (mirrored column); avoid auth.users in RPC.
...
comment on function public.contest_leaderboard(text) is
  'Leaderboard rows with email from profiles, entry_number per user in contest; ORDER BY total_score DESC.';
