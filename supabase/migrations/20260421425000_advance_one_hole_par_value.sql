-- v_par_value uses get_hole_par(sl.hole+1) for outcome weights; hole_par column synced each tick via get_hole_par(hole).
-- Requires get_hole_par (214240) and sim_live_state.hole_par (214230).
...
begin
  ...
  loop
    ...
    v_par_value := public.get_hole_par(sl.hole + 1);
    ...
    -- Par difficulty by hole par (after skill/weekend, before jitter)
    if v_par_value = 5 then
      v_birdie := v_birdie + 0.05;
      v_bogey := v_bogey - 0.03;
    end if;

    if v_par_value = 3 then
      v_birdie := v_birdie - 0.04;
      v_bogey := v_bogey + 0.04;
    end if;

    v_birdie := greatest(0.08::numeric, least(0.40::numeric, v_birdie));
    v_bogey := greatest(0.05::numeric, least(0.35::numeric, v_bogey));
  ...
  update public.sim_live_state
  set hole_par = public.get_hole_par(hole);
...
comment on function public.advance_one_hole() is
  'Weights use get_hole_par(sl.hole+1); end tick sets hole_par=get_hole_par(hole) on all rows; apply_cut R2.';
