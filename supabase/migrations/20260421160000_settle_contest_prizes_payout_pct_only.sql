-- Settlement uses contest_payouts.payout_pct only. Validate sum(expected_pool × payout_pct / 100).
-- Remove optional legacy dollar column on contest_payouts if a prior migration added it.

alter table public.contest_payouts drop column if exists payout_amount;
