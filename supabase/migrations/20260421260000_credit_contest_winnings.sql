-- Wallet credits from contest_entry_results (after run_contest_payouts). Idempotent per contest.
...
create table if not exists public.contest_winnings_credits (
  contest_id text primary key,
  credited_at timestamptz not null default now()
);

comment on table public.contest_winnings_credits is
  'One row per contest after credit_contest_winnings has applied wallet credits (idempotency).';

alter table public.contest_winnings_credits owner to postgres;

alter table public.contest_winnings_credits enable row level security;

grant all on table public.contest_winnings_credits to service_role;
...
comment on function public.credit_contest_winnings(text) is
  'Credits profiles.account_balance from contest_entry_results; one contest_winnings_credits row per contest. Idempotent.';

grant all on function public.credit_contest_winnings(text) to anon;
grant all on function public.credit_contest_winnings(text) to authenticated;
grant all on function public.credit_contest_winnings(text) to service_role;
