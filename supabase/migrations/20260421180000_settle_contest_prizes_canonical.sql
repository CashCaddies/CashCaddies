-- Single canonical settle_contest_prizes(text): jsonb RPC for the app, payout_pct only, created_at ranking.
-- Drops any text/uuid overloads. Prize pool = 90% of sum(contest_entries.entry_fee). No score / payout_amount.
...
begin
  ...
  -- 90% of total entry fees collected on entries (numeric; coalesce null fees to 0)
  select coalesce(sum(coalesce(ce.entry_fee::numeric, 0)), 0) * 0.90
  into expected_pool
  ...
  from (
    select
      ce.user_id,
      row_number() over (
        order by ce.created_at asc nulls last, ce.id asc
      ) as ord
    from public.contest_entries ce
    where ce.contest_id = v_cid::uuid
  ) ranked;
...
comment on function public.settle_contest_prizes(text) is
  'Prize pool = 90% of sum(entry_fee) on entries; ranks by contest_entries.created_at asc; contest_payouts.payout_pct only; credits wallets; one contest_settlements row. Idempotent.';
