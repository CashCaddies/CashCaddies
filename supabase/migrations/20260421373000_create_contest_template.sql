-- Admin RPC to insert a contest template (SECURITY DEFINER; enforces staff role in-function).
...
) is 'Insert a contest_templates row. Caller must be service_role or admin/senior_admin/founder.';

revoke all on function public.create_contest_template(
  text, text, integer, integer, integer, text, integer, numeric, jsonb, text
) from public;

grant execute on function public.create_contest_template(
  text, text, integer, integer, integer, text, integer, numeric, jsonb, text
) to authenticated;
