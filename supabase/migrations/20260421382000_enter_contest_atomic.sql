-- Thin entry helper: validates lobby contest + capacity, then delegates to create_contest_entry_atomic.
-- CashCaddies has no `entries`, `wallets`, or `balance_cents`; use contest_entries + profiles.account_balance.
-- Pass p_lineup as {"lineup_id": "<uuid>"} pointing at an existing public.lineups row (roster JSON is not stored on entries).
