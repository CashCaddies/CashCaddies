-- Per-row paid flags (set when credit_contest_winnings runs) + FK for profile embeds.

alter table public.contest_entry_results
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'contest_entry_results_user_id_fkey'
  ) then
    alter table public.contest_entry_results
      add constraint contest_entry_results_user_id_fkey
      foreign key (user_id) references public.profiles (id);
  end if;
end $$;

-- Backfill from contests that already completed wallet credits
update public.contest_entry_results cer
set
  paid = true,
  paid_at = w.credited_at
from public.contest_winnings_credits w
where trim(cer.contest_id) = trim(w.contest_id)
  and cer.paid is distinct from true;
