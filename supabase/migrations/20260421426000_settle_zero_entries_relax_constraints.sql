-- Idempotent: allow contest_settlements rows with entry_count = 0 (empty contests).
-- Baseline used entry_count >= 1; 20260411260000 relaxed to >= 0 — re-apply for any DB that missed it.

alter table public.contest_settlements drop constraint if exists contest_settlements_entry_count_check;

alter table public.contest_settlements
  add constraint contest_settlements_entry_count_check check (entry_count >= 0);

-- run_contest_payouts: after settlement, zero entries means nothing to allocate — succeed without requiring contest_payouts rows.
