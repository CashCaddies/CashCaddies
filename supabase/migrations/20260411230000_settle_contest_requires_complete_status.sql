-- Settlement aligns with lifecycle: only when contests.status = 'complete' (not a fixed delay after start).
-- Prize pool remains entry_fee_usd * entry_count * 0.90 (10% platform + protection via triggers).
...
comment on function public.settle_contest_prizes(text) is
  'Requires contests.status = complete. Sorts leaderboard by lineup total_score, applies contest_payouts % of prize pool (90% of gross entry fees), credits account_balance and transactions. Idempotent via contest_settlements.';
