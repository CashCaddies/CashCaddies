-- Re-apply settle_contest_prizes: prize order uses contest_entries.created_at asc (then id), not lineup total_score,
-- so settlement runs before a scoring system exists.
...
comment on function public.settle_contest_prizes(text) is
  'Order entries by contest_entries.created_at asc (then id), apply contest_payouts, credit account_balance and transactions (idempotent per contest). Rejects settlement if any prize would exceed beta wallet cap (5000 USD).';
